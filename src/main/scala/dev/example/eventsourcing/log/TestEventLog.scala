...
class TestEventLog[E <: Event] extends EventLog[E] {
  ...
  val eventStore = Actor.actorOf(new TestEventStore(eventsRef, storedEvents)).start
...
class TestEventStore[E <: Event](val eventsRef: Ref[List[E]], stored: CopyOnWriteArrayList[E]) extends Actor with EventStore[E] {

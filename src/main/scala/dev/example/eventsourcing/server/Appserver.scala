...
object Appserver {
  def boot(): Appserver = new Appserver {
    val eventLog = new DefaultEventLog with EventLoggedNotification { val eventBus = new DefaultEventBus }
    ...
    eventLog.eventBus.subscribe(invoiceReplicator, "log")
    eventLog.eventBus.subscribe(invoiceStatistics, "log")
  }
}